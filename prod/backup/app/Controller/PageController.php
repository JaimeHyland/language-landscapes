...
http://language-landscapes.com/dev/files/documentToQuote/" . $this->Session->id() . "/$entry";
				...
				if(isset($this->data['Contact']['sendCopyToClient'])) {
					$Email->to($this->data['Contact']['email']);
					$Email->send($msg);
				}
	...
	public function sendFormWorkWithUs() {
		$this->loadModel('WorkWithUs');

		$this->set('sid', $this->Session->id());

		if($this->request->isPost() && !empty($this->data)) {
			$this->WorkWithUs->set($this->data);
			if( $this->WorkWithUs->validates() ){
				$msg = "
Form information:
firstName: ".$this->data['WorkWithUs']['firstName']."
lastName: ".$this->data['WorkWithUs']['lastName']."
email: ".$this->data['WorkWithUs']['email']."
translation: ".$this->data['WorkWithUs']['translation']."
localization: ".$this->data['WorkWithUs']['localization']."
copywriting: ".$this->data['WorkWithUs']['copywriting']."
optimization: ".$this->data['WorkWithUs']['optimization']."
internationalization: ".$this->data['WorkWithUs']['internationalization']."
langpairs:
".$this->data['WorkWithUs']['langpairs']."
message: ".$this->data['WorkWithUs']['message']."

Files attached:
";

				// send email with links to files
				if (file_exists(WWW_ROOT . 'files/documentCV/' . $this->Session->id()) && $handle = opendir(WWW_ROOT . 'files/documentCV/' . $this->Session->id() )) {

				    /* This is the correct way to loop over the directory. */
				    while (false !== ($entry = readdir($handle))) {
				    	if($entry != '.' && $entry != '..')
						$msg .= "
http://language-landscapes.com/dev/files/documentCV/" . $this->Session->id() . "/$entry";
				    }
				...
				$Email->from(array('info@language-landscapes.com' => 'Language Landscapes'));
				$Email->to('info@language-landscapes.com');
				$Email->subject('Language Landscapes: new CV');
				session_regenerate_id();
				$Email->send($msg);
				...
				$errors = $this->WorkWithUs->validationErrors;
				$errorsMsg = "";
				foreach ($errors as $key => $value) {
					$errorsMsg .= implode('<br>', $value);
				}
