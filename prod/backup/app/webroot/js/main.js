$(document).ready(function(){
	...
	function open_form(el, direction, id) {
		switch(direction) {
			case 'left':
				$(id).animate({
					left:"0px"
	...
	function close_form(el, direction, id) {
		width = $(id).width();
		padding = $(id).css('padding-left');
	...
	$('.vertical_button_left').on("click",function(){
		...
		opened_left ? open_form(this, 'left', '#sliding_form_1') : close_form(this, 'left', '#sliding_form_1');
	})

	$('.vertical_button_right').on("click",function(){
		opened_right = opened_right ? false : true;
		opened_right ? open_form(this, 'right', '#sliding_form_2') : close_form(this, 'right', '#sliding_form_2');
	})
